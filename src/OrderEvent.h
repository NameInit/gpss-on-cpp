...
class OrderEvent{
    ...
    public:
        ...
        friend std::ostream& operator<<(std::ostream& output, const OrderEvent& status);
		friend std::ofstream& operator<<(std::ofstream& output, const OrderEvent& status);
};
...
std::ofstream& operator<<(std::ofstream& output, const OrderEvent& status){
    output<<status._all_status[status._ind_cur_status];
    return output;
}

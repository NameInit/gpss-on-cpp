...
int main() {
  ...
  if (count_type_tranzakt < count_type_worker) {
    throw std::logic_error(
        "count_type_tranzakt shouldn't be less count_type_worker");
  }
  ...
  int **mean_processing_tranzakt =
      get_mean_processing_tranzakt(count_type_tranzakt, count_type_worker);

  Simulation thread1(count_type_tranzakt, count_type_worker, mean_born_tranzakt,
                     count_workers, mean_processing_tranzakt);
  thread1.start_simulation(logger, COUNT_TRANZAKTS_SIMULATION);

  clear(mean_born_tranzakt, count_workers, mean_processing_tranzakt,
        count_type_worker);
  return 0;
}
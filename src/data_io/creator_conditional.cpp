...
void show_conditional(int *mean_born_tranzakt, int *count_workers,
  ...
  for (int i = 0; i < count_type_tranzakt; i++)
    std::cout << "Tranzakt type=" << i + 1 << ": " << mean_born_tranzakt[i]
              << std::endl;

  std::cout << std::endl << "-----COUNT WORKERS-----" << std::endl;
  for (int i = 0; i < count_type_worker; i++)
    std::cout << "Worker type=" << i + 1 << ": " << count_workers[i]
              << std::endl;
  ...
  for (int i = 0; i < count_type_worker; i++) {
    ...
    for (int j = 0; j < count_type_tranzakt; j++) {
      std::cout << "Tranzakt " << j + 1 << ": "
                << mean_processing_tranzakt[i][j] << std::endl;
    }
    if ((i + 1) != count_type_worker)
      std::cout << std::endl;
  }
}
...
int **get_mean_processing_tranzakt(int count_type_tranzakt,
  ...
  for (int i = 0; i < count_type_worker; i++) {
    ...
    for (int j = 0; j < count_type_tranzakt; j++) {
      ...
      if (mean_processing_tranzakt[i][j] != None)
        flag_input = false;
    }
    if (flag_input) {

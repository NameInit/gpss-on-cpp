#include "../headers/CurrentEventsChain.h"
...
unsigned CurrentEventsChain::size() { return list.size(); }

Tranzakt *CurrentEventsChain::operator[](int n) {
  if (n < 0 || n >= list.size()) {
    throw std::logic_error(
        "index out of range in CurrentEventsChain::operator[]");
  }
  return list[n];
}
...
void CurrentEventsChain::push(Tranzakt *tranzakt) { list.push_tail(tranzakt); }

bool CurrentEventsChain::can_promote(MyStorage *storage) {
  for (int i = 0; i < list.size(); i++) {
    if (!(list[i]->current_block() == (const char *)"ENTER" &&
          storage[list[i]->index_blocks()].count_free_space() == 0))
      return true;
  }
  return false;
}
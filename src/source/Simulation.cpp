...
Simulation::Simulation(int count_type_tranzakt, int count_type_worker,
                       int *mean_born_tranzakt, int *count_workers,
                       int **mean_processing_tranzakt)
    : __current_id(0), __current_time(0.),
      __count_type_tranzakt(count_type_tranzakt),
      __count_type_worker(count_type_worker) {
...
void Simulation::start_simulation(std::ofstream &logger,
                                  int count_tranz_for_terminate) {
  if (count_tranz_for_terminate <= 0) {
    throw std::logic_error(
        "count tranzakts for terminate must be positive integer");
  }
  bool flag_exit = true;
...
void Simulation::__ViewingPhase() {
  ...
  for (int i = 0; i < __CEC.size() * (!flag_exit); i++) {
    if (__CEC[i]->current_block() == (const char *)"START") {
      ...
      if (__CEC[i]->index_blocks() == None)
        __CEC[i]->index_blocks(necessary_type - 1);
      ...
      if (!__advance[__CEC[i]->index_blocks()].is_passed(__CEC[i])) {
        ...
        for (int i = 0; i < __CEC.size(); i++)
          if (__CEC[i]->time() < __current_time)
            temp[__CEC[i]->index_blocks()] += 1;
        for (int i = 0; i < __count_type_worker; i++)
          if (__max_len_queue[i] < temp[i])
            __max_len_queue[i] = temp[i];
        __sum_time_in_queue[__CEC[i]->index_blocks()] +=
            __current_time - __CEC[i]->time();
        delete[] temp;
        // end
...
void Simulation::__TimerCorrectionPhase() {
  ...
  for (int i = 0; i < __FEC.size(); i++) {
    if (__current_time != __FEC[0]->time())
      break;
...
void Simulation::__WriteStatistic(std::ofstream &logger) {
  ...
  for (int i = 0; i < __count_type_worker; i++) {
    logger << std::setw(4) << ' ';
    logger << (const char *)"Queue " << i + 1 << (const char *)": "
           << __sum_time_in_queue[i] / __current_time
           << (const char *)" (MAX_LEN=" << __max_len_queue[i] << ')'
           << std::endl;
  }
  logger << std::left;
}
...
void Simulation::__WriteInFile(std::ofstream &logger, const char *phase) {
  ...
  for (int i = 0; i < __count_type_worker; i++) {
    logger << std::setw(4) << ' ';
    logger << std::setw(4) << (const char *)"Workers type=" << i + 1
           << (const char *)" have " << __storages[i].count_free_space()
           << (const char *)" places" << std::endl;
  }
  logger << std::left;

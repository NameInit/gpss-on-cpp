...
template <typename T> void List<T>::push(T &data, int n) {
  if (n < 0 || n > this->_size) {
...
template <typename T> unsigned List<T>::size() { return this->_size; }
...
template <typename T> T List<T>::pop_tail() {
  T data = __tail->data;
...
template <typename T> unsigned List<T>::index(T &data) {
  Node *temp = __head;
  int ind = 0;
  while (temp) {
    if (temp->data == data)
...
template <typename T> T List<T>::pop(int n) {
...
template <typename T> void List<T>::push_tail(T &data) {
  this->_size += 1;
  if (!__head) {
    __head = new Node;
    __head->data = data;
